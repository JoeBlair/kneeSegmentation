...
l_conv_1 = lasagne.layers.dnn.Conv3DDNNLayer(input, 20, (2,2,2))

#l_maxpool_1 = lasagne.layers.dnn.MaxPool3DDNNLayer(l_conv_1, (2,2,2))

l_conv_2 = lasagne.layers.dnn.Conv3DDNNLayer(l_conv_1, 20, (2,2,2))
...
l_conv_11 = lasagne.layers.dnn.Conv3DDNNLayer(input3, 20, (2,2,2))

#l_maxpool_1 = lasagne.layers.dnn.MaxPool3DDNNLayer(l_conv_1, (2,2,2))

#l_conv_2 = lasagne.layers.dnn.Conv3DDNNLayer(l_maxpool_1, 20, (2,2,2))
...
updates = lasagne.updates.nesterov_momentum(all_grads, all_params, learning_rate=0.001, momentum=0.75)
...
Train = DP.get_paths("/home/xvt131/Running/small_set")
Test = DP.get_paths("/home/xvt131/Running/vtest")
...
for epoch in range(num_epochs):
    ...
    for im in Train:
        #print counter
        [X_train, X_Bigger], X_pos, Y_train  = DP.voxel_samples(im, [9, 5])
    ...
    for img in Test:
	    ...
	    Tibia, Femur, Time = E.Evaluate1("/home/xvt131/Running/vtest", DP.image_load, 9, 5 ,f_eval)

            print "Mean Tibia Dice Score:" , np.mean(Tibia)
            print "Mean Femur Dice Score:", np.mean(Femur)
...
X, Y, Z = E.Evaluate1("/home/xvt131/Running/validating", DP.image_load, 9, 5 ,f_eval)
