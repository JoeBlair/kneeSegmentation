...
def Sampling(image_path):
    ...
    Dims = np.prod(Image['goldROI'].shape)
    
    Label = np.zeros(Image['goldROI'].shape)
...
def Patch_gen_mix(indeces, patch_size, A):
    ...
    n3 = (15 -1)/2
    m3 = 15
    image_pad3 = np.pad(A['scanROI'], pad_width=n3, mode='constant', constant_values = 0)

    A3 = Parallel(n_jobs= 1)(delayed(frame)(i, image_pad3, m3) for i in indeces)
    A3 = np.array(A3)

    X, Y, Z = np.hsplit(A3, 3)
...
def Patch_mixed_para(image_path, patch_size):
    ...
    image_pad2 = np.pad(Image, pad_width=n3, mode='constant', constant_values = 0)

    A3 = Parallel(n_jobs= 1)(delayed(frame)(i, image_pad2, m3) for i in indeces)
    A3 = np.array(A3)

    X, Y, Z = np.hsplit(A3, 3)


    return XY, XZ, YZ, X, Y, Z, Label
