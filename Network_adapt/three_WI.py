...
l_conv_2 = lasagne.layers.dnn.Conv3DDNNLayer(l_maxpool_1, 40, (7,7,7))

l_conv_3 = lasagne.layers.dnn.Conv3DDNNLayer(l_conv_2, 80, (5,5,5))
...
for epoch in range(num_epochs):
   ...
   # Train = np.random.choice(Train_all, 5)
    ...
    if (epoch) % 10 == 0:
        ...
        np.savez('/home/xvt131/Functions/Adhish_copy/three_param/epoch_%d_params_bigger.npz' %(epoch), *lasagne.layers.get_all_param_values(output))


    print confusion_train
    print "Epoch %i : Train Loss %e , Train acc %f,  Valid_loss %f, Valid acc %f " % (epoch+1, loss[-1], train_acc_cur, valid_loss[-1] ,valid_acc_cur)


np.savez('bigger_filt.npz', *lasagne.layers.get_all_param_values(output))

np.savez('Loss_value_BF.npz', loss, train_acc, valid_loss, valid_acc)
