...
# Spatial Prior FFNet
input2 = lasagne.layers.InputLayer((None, 1, 6), input_var=input2_var)
...
l_hidden_spa4 = lasagne.layers.DenseLayer(l_hidden_spa3, num_units=32, nonlinearity=lasagne.nonlinearities.rectify,W=lasagne.init.HeNormal(gain='relu'))

l_hidden_spa5 = lasagne.layers.DenseLayer(l_hidden_spa4, num_units=16, nonlinearity=lasagne.nonlinearities.rectify,W=lasagne.init.HeNormal(gain='relu'))

l_hidden_spa6 = lasagne.layers.DenseLayer(l_hidden_spa5, num_units=8, nonlinearity=lasagne.nonlinearities.rectify,W=lasagne.init.HeNormal(gain='relu'))

l_hidden_spa7 = lasagne.layers.DenseLayer(l_hidden_spa6, num_units=4, nonlinearity=lasagne.nonlinearities.rectify,W=lasagne.init.HeNormal(gain='relu'))


#Output Layer
output = lasagne.layers.DenseLayer(l_hidden_spa7, num_units=classes, nonlinearity = lasagne.nonlinearities.softmax)
...
for epoch in range(num_epochs):
    ...
    for img in Test:
        Scan, Y_test,A, Pos  = DP.Sampling(im)
...
np.savez('WI_Params_1.npz', *lasagne.layers.get_all_param_values(output))
