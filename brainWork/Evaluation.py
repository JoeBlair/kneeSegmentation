...
def Evaluate(paths, data_load_fun, Patches):
    ...
    for img in Paths:
        ...
        Lab, Posi, Pat, Sha =  data_load_fun(img, Patches)
...
def Evaluate2(paths, data_load_fun, Patch_Size, eval_func):
    ...
    for img in Paths:
        print img
        Image = io.loadmat(img)
        Sha = Image['scanROI'].shape
        XY, XZ, YZ, Lab =  data_load_fun(img, Patch_Size)
        batch_size = 2048
        preds = np.zeros(shape = ( len(Lab), 3))
        size_im = len(Lab)
        num_batches = size_im // batch_size
...
#
 #       Gold_Dice[i] = Dice_score(Final_pred, Lab, 1)
  ...
  #      Silver_Dice[i] = Dice_score(Final_pred, Lab, 2)
      ...
      #  np.savez("Segmentatin_%s" %(img[-7:-4]), Final_pred, Lab)
   #     io.savemat("/home/xvt131/Functions/Adhish_copy/Segs/Seg_%s" %(img[-7:-4]), mdict= {"Seg_%s" %(img[-7:-4]):Final_pred, "Lab_%s" %(img[-7:-4]):Lab, Gold_Dice[i]:Lab1_dice, Silver_Dice[i]:Lab2_Dice})
        print i
...
def Segment(paths, data_load_fun,  Patch_Size,Patch2, eval_func):
    ...
    for img in Paths:
        Image = io.loadmat(img)
        Nd, Posi, Pat ,Pat2, Sha =  data_load_fun(img, Patch_Size, Patch2)
        batch_size = 2048
